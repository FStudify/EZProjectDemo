...
const ROLE_ICONS: Record<ProjectRole, { Icon: typeof Flame; label: string }> = {
  leader: { Icon: Flame, label: 'Leader' },
  supervisor: { Icon: BookOpen, label: 'Supervisor' },
  member: { Icon: Pickaxe, label: 'Member' },
};
...
export default function MemberAvatar({
  ...
  const { Icon, label } = ROLE_ICONS[role];
  const ringClass = online ? 'ring-2 ring-emerald-500' : 'ring-2 ring-slate-300';
  const tooltipLines = [name, isOwner && 'Owner', label].filter(Boolean).join('\n');
  ...
  return (
    <div className="relative inline-block group" title={tooltipLines}>
      ...
      >
        ...
        {isOwner && <p className="text-amber-300">Owner</p>}
